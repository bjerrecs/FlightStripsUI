...
export const Strip = (props) => {
  return (
    <div className="w-[90%] h-14 bg-[#BEF5EF] border-l-4 border-r-4 border-t-2 border-b-2 mb-1 flex items-center ">
        ...
        <div className="w-[15%] border-r-1 border-l-1 border-t-2 border-b-2 border-[#85B4AF] pl-4 pr-4 h-full flex flex-col  items-center justify-center text-center">
            <span className="font-bold">{props.ades}</span><span className="font-bold">{props.stand}</span>
        </div>
        <div className="w-[30%] border-r-1 border-l-1 border-t-2 border-b-2 border-[#85B4AF] h-full flex items-top justify-between text-center pl-2 pr-2 whitespace-nowrap">
            <div>EOBT: {props.eobt}</div>
        </div>
